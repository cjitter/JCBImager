    ...
    : pluginFormat(format)
{
    ...
    // Cargar imagen de fondo genérica si existe
    backgroundImage = juce::ImageCache::getFromMemory(BinaryData::fondo_png, 
                                                     BinaryData::fondo_pngSize);
...
void CreditsWindow::paint(juce::Graphics& g)
{
    ...
    for (int lineIndex = 0; lineIndex < static_cast<int>(lines.size()); ++lineIndex)
    {
        ...
        bool isTitle = (lineIndex == 1);  // "JCBImager v1.0.0-alpha.1" es la segunda línea
        if (isTitle)
        {
...
void CreditsWindow::recalculateLinkBounds()
{
    ...
    for (const auto& line : lines)
    {
        ...
        // Espaciado adicional después del título
        bool isTitle = line.contains("JCBImager v");
        if (isTitle)
        {
