      ...
      currentProgram(-1)
{
    ...
    for (const auto& entry : genParameterList)
        apvts.addParameterListener(entry.first, this);
    ...
    for (const auto& entry : genParameterList)
    {
        ...
        if (auto* p = apvts.getParameter(paramName))
        {
            ...
            else if (auto* raw = apvts.getRawParameterValue(paramName))
                v = raw->load();
            JCBImager::setparameter(m_PluginState, idx, v, nullptr);
        }
    }
}
...
JCBImagerAudioProcessor::~JCBImagerAudioProcessor()
{
    ...
    for (const auto& entry : genParameterList)
        apvts.removeParameterListener(entry.first, this);
...
void JCBImagerAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    for (const auto& entry : genParameterList)
    {
        if (auto* param = apvts.getRawParameterValue(entry.first)) {
            JCBImager::setparameter(m_PluginState, entry.second, param->load(), nullptr);
        }
    }
...
void JCBImagerAudioProcessor::parameterChanged(const juce::String& parameterID, float newValue)
{
    ...
    if (m_PluginState != nullptr)
    {
        auto it = genIndexByName.find(parameterID);
        if (it != genIndexByName.end())
            JCBImager::setparameter(m_PluginState, it->second, v, nullptr);
    }
...
//==============================================================================
void JCBImagerAudioProcessor::enqueueAllParametersForAudioThread() noexcept
{
...
void JCBImagerAudioProcessor::rebuildGenParameterLookup()
{
    ...
    const int totalParams = JCBImager::num_params();
    genIndexByName.reserve((size_t) totalParams);
    genParameterList.reserve((size_t) totalParams);
...
void JCBImagerAudioProcessor::setParameter(int index, float newValue)
{
    auto& params = getParameters();
    if (juce::isPositiveAndBelow(index, params.size()))
        params.getUnchecked(index)->setValueNotifyingHost(newValue);
}
...
const juce::String JCBImagerAudioProcessor::getParameterName(int index)
{
    auto& params = getParameters();
    if (juce::isPositiveAndBelow(index, params.size()))
        return params.getUnchecked(index)->getName(64);
    return {};
}
...
const juce::String JCBImagerAudioProcessor::getParameterText(int index)
{
    auto& params = getParameters();
    if (juce::isPositiveAndBelow(index, params.size()))
        return params.getUnchecked(index)->getText(params.getUnchecked(index)->getValue(), 64);
    return {};
}
