      ...
      outputMeterR([]() { return -100.0f; })      // Safe dummy value for output meters
{
    ...
    if (hostInfo.isLogic()) {
        ...
        titleText = "JCBImager v1.0.0";  // Nombre completo para otros DAWs
    }
    ...
    // No registrar listeners de parámetros legacy que ya no existen en Imager
    
    // Connect spectrum analyzer callback to processor
    auto analyzerSafe = juce::Component::SafePointer<SpectrumAnalyzerComponent>(&spectrumAnalyzer);
    processor.setSpectrumAnalyzerCallbackStereo([analyzerSafe](float l, float r) {
        if (analyzerSafe != nullptr)
            analyzerSafe->pushNextStereoSample(l, r);
    });
    
    // Connect sample rate change callback
    processor.setSampleRateChangedCallback([analyzerSafe](double newSampleRate) {
        if (analyzerSafe != nullptr)
            analyzerSafe->setSampleRate(newSampleRate);
    });
...
void JCBImagerAudioProcessorEditor::resized()
{
    ...
    const int lowCenterX = midCenterX - bandSpacing + 4;
    const int highCenterX = midCenterX + bandSpacing - 4;
    ...
    imager.inputMode.setBounds(getScaledBounds(165, 45, 20, 12));
    imager.outputMode.setBounds(getScaledBounds(512, 45, 25, 12));
    ...
    // Panel derecho legacy removido; área reservada para futuros medidores
...
void JCBImagerAudioProcessorEditor::buttonClicked(juce::Button* button)
{
    ...
    else if (button == &parameterButtons.bypassButton)
    {
        ...
        if (auto* bypassParam = processor.apvts.getParameter("i_BYPASS"))
        {
            bypassParam->beginChangeGesture();
            bypassParam->setValueNotifyingHost(bypassActive ? 1.0f : 0.0f);
            bypassParam->endChangeGesture();
        }

        processor.pushGenParamByName("i_BYPASS", bypassActive ? 1.f : 0.f);
...
void JCBImagerAudioProcessorEditor::handleParameterChange()
{
    ...
    if (presetArea.presetMenu.getSelectedId() > 0) {
        ...
        // Siempre considerar que ningún preset está activo
        processor.setLastPreset(0);
...
{
    // Ocultar todos los grupos heredados que ya no aplican (Reverb/Filters)
...
void JCBImagerAudioProcessorEditor::setupPresetArea()
{
    ...
    presetArea.presetMenu.onChange = [this]() {
        ...
        if (presetName == "DEFAULT") {
            ...
            processor.enqueueAllParametersForAudioThread();
        }
...
{
    // Imager no utiliza botones de orden para los filtros heredados.
}
...
void JCBImagerAudioProcessorEditor::updateButtonValues()
{
    ...
    // Bypass button (interno Gen: i_BYPASS)
    {
        ...
        if (auto* value = processor.apvts.getRawParameterValue("i_BYPASS"))
            toggleState = (value->load() >= 0.5f);
    ...
    showCustomConfirmDialog(JUCE_UTF8("Delete Preset"),
        ...
        if (confirmed) {
            ...
            if (presetFile.existsAsFile()) {
                ...
                // Actualizar el menú
                processor.setLastPreset(0);
                presetArea.presetMenu.setSelectedIdWithoutNotification(0);
                refreshPresetMenu();
...
juce::String JCBImagerAudioProcessorEditor::getTooltipText(const juce::String& key)
{
    if (currentLanguage == TooltipLanguage::Spanish)
    {
        ...
        if (key == "title") return JUCE_UTF8("JCBImager v1.0.0\nPlugin de audio open source\nClick para créditos");
...
int JCBImagerAudioProcessorEditor::getControlParameterIndex(juce::Component& control)
{
    ...
    else if (&control == &trimSlider)         parameterID = "v_TRIM";
    else if (&control == &makeupSlider)       parameterID = "w_MAKEUP";
